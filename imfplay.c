...
void opl2_play_imf_file(char *fname, float freq) {
    ...
    for(; record < song_size; record += sizeof(t_sound_data)) {
        ...
        if(sound_data->delay) {
            wait = ((float)sound_data->delay / freq) * 1000.0;
            delay((uint16_t) wait);      
        }
    }
...
int main(char **argv, int argc) {
    ...
    puts("IMFPLAY v0.1 - (c)2017/2018 bootsector - http://www.brunofreitas.com/\r\n\r\n");
    ...
    opl2_play_imf_file(argv[0], (float) frequency);
